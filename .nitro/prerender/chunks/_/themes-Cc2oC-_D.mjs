...
var _tmpl$ = ["<div", ' class="px-4 sm:px-6 py-12 sm:py-20 max-w-6xl mx-auto"><div class="mb-12"><h1 class="text-4xl sm:text-5xl font-black text-white mb-4">Themes</h1><p class="text-fg text-lg">Zenkai ships with <span class="text-primary font-semibold"><!--$-->', '<!--/--> built-in themes</span>. Browse the gallery below, then launch with <code class="text-primary">zenkai --theme=&lt;name&gt;</code>.</p></div><div class="flex flex-col gap-3">', "</div></div>"], _tmpl$2 = ["<img", ' src="', '"', ' class="w-48 shrink-0 object-cover" loading="lazy">'], _tmpl$3 = ["<div", ' class="p-4 flex flex-col gap-2 justify-center flex-1"><div class="flex items-center gap-3"><h3 class="font-bold text-white">', '</h3><span class="text-sm text-muted">', '</span></div><pre class="bg-base-400/50 rounded px-2 py-1.5 text-xs font-mono text-fg overflow-x-auto inline-block w-fit">zenkai --theme=<!--$-->', "<!--/--></pre></div>"];
  ...
  return createComponent(DocsLayout, {
    get children() {
      return ssr(_tmpl$, ssrHydrationKey(), escape(themes.length), escape(themes.map((t) => createComponent(Card, {
        "class": "overflow-hidden flex flex-row items-stretch",
        get children() {
...
//# sourceMappingURL=themes-Cc2oC-_D.mjs.map
